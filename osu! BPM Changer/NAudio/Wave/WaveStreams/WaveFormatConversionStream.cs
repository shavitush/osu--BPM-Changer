...
namespace NAudio.Wave
{
    ...
    public class WaveFormatConversionStream : WaveStream
    {
        ...
        public WaveFormatConversionStream(WaveFormat targetFormat, WaveStream sourceStream)
        {
            ...
            position = 0;
            preferredSourceReadSize = Math.Min(sourceStream.WaveFormat.AverageBytesPerSecond,
        ...
        public override long Position
        {
            ...
            set
            {
                ...
                long desiredSourcePosition = EstimateDestToSource(value); //conversionStream.DestToSource((int) value); 
                sourceStream.Position = desiredSourcePosition;
                position = EstimateSourceToDest(sourceStream.Position);
                    //conversionStream.SourceToDest((int)sourceStream.Position);
                leftoverDestBytes = 0;
        ...
        {
            long dest = ((source*targetFormat.AverageBytesPerSecond)/sourceStream.WaveFormat.AverageBytesPerSecond);
        ...
        {
            long source = ((dest*sourceStream.WaveFormat.AverageBytesPerSecond)/targetFormat.AverageBytesPerSecond);
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (bytesRead < count)
            {
                ...
                else
                {
                    // possible error here
                    Debug.WriteLine(
                        string.Format("sourceBytesRead: {0}, sourceBytesConverted {1}, destBytesConverted {2}",
