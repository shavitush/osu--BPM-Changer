...
namespace NAudio.Wave
{
    ...
    public class MediaFoundationResampler : MediaFoundationTransform
    {
        ...
        {
            IEnumerable<IMFActivate> transformActivators =
                MediaFoundationApi.EnumerateTransforms(MediaFoundationTransformCategories.AudioEffect);
            foreach (IMFActivate activator in transformActivators)
            {
                Guid clsid;
        ...
        {
            object comObject = CreateResamplerComObject(); // new ResamplerMediaComObject();
            var resamplerTransform = (IMFTransform) comObject;

            IMFMediaType inputMediaFormat = MediaFoundationApi.CreateMediaTypeFromWaveFormat(sourceProvider.WaveFormat);
